        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('translations.create') }}" class="btn btn-success float-end">
                        Create
                    </a>
                </div>
                <div class="card-body">
                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap w-100">
                            ...
                            @foreach($translations as $translation)
                                <tr>
                                    ...
                                    <td>{!! \Illuminate\Support\Str::words(($translation->quranVerse->verse_arabic ?? ''),8,'...') !!}</td>
                                    <td>{{ $translation->quranTranslationProvider->brand_name ?? '' }}</td>
                                    <td>{!! \Illuminate\Support\Str::words($translation->translated_verse,8,'...') !!}</td>
...
{{--                                           <a href="{{route('translation.show',$translation->slug)}}" class=" btn btn-primary btn-sm py-0 px-1"><i class="uil-eye"></i></a>--}}
                                            <a href="{{ route('translations.edit',$translation->slug) }}" class="mx-1 btn btn-info btn-sm py-0 px-1"><i class="dripicons-document-edit"></i></a>
                                            <form action="{{ route('translations.destroy',$translation->slug) }}" method="post" style="display: inline-block"  onsubmit="return confirm('Are You Sure That To Delete This Translation'); ">
                                                @csrf
                                                @method('delete')
