                            ...
                            @foreach($verses as $verse)
                                <tr>
                                   ...
                                   <td>{{ $verse->quranFont->quran_font }}</td>
                                    <td>{!! \Illuminate\Support\Str::words($verse->verse_arabic,8,'...') !!}</td>
                                    <td>{!! \Illuminate\Support\Str::words($verse->verse_bangla,8,'...') !!}</td>
                                    <td>{!! \Illuminate\Support\Str::words($verse->verse_english,8,'...') !!}</td>
                                    <td>
...
{{--                                            <a href="{{route('varse.show',$varse->slug)}}" class=" btn btn-primary btn-sm py-0 px-1"><i class="uil-eye"></i></a>--}}
                                            <a href="{{ route('verses.edit',$verse->slug) }}" class="mx-1 btn btn-info btn-sm py-0 px-1"><i class="dripicons-document-edit"></i></a>
                                            <form action="{{ route('verses.destroy',$verse->slug) }}" method="post" style="display: inline-block"  onsubmit="return confirm('Are You Sure That To Delete This Verse'); ">
                                                @csrf
                                                @method('delete')
